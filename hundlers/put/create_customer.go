...
type CreateCustomerResponseSuccess struct {
	Message    string                `json:"message"`
	Request    CreateCustomerRequest `json:"request"`
	Registered db.CustomerModel      `json:"registered"`
}

type CreateCustomerResponseFailure struct {
	Message string                `json:"message"`
	Request CreateCustomerRequest `json:"request"`
}
...
func CreateCustomer(w http.ResponseWriter, r *http.Request) {
	...
	defer func() {
		...
		if status == http.StatusOK {
			res := new(CreateCustomerResponseSuccess)
			...
			res := new(CreateCustomerResponseFailure)
		...
		fmt.Printf("[Create Customer][%d] %s\n", status, message)
	...
	// 顧客情報の挿入
	created, err := client.Customer.CreateOne(
